...
| CLI verifier (`cigp verify`) | `cli/cigp` | **IMPLEMENTED** — JSON proof verification with meaningful exit codes |
| Demo slot simulator | `simulator/slot` | **IMPLEMENTED** — deterministic virtual-credit reference game |
| Statistical engine | `julia/CIGPStatistics.jl` | **REFERENCE** — RTP, frequencies, and confidence interval utilities |
| Anomaly research layer | `python/cigp_anomaly` | **REFERENCE** — deterministic z-score classification |
| Browser verifier | `typescript/cigp-verifier` | **REFERENCE** — protocol gate and SHA-256 helper |
| Cross-language test vectors | `test-vectors/` | **IMPLEMENTED** — generated by the Rust reference CLI |
...
Note that `Signature: PASS` can still hold against a proof whose content was tampered with, because the Ed25519 signature is checked against the proof's stored `round_hash`; the independently recomputed `Round hash: FAIL` check catches the inconsistency. `VerificationReport::is_valid()` requires every check to pass, so the overall result is correctly `INVALID`. The reference CLI now exposes this behavior through `cigp verify`.
